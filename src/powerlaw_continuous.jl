...
mean(d::ContinuousPowerLaw) = ((α, θ) = params(d); α > 2.0 ? θ *((α - 1.0)/(α - 2.0)) : Inf)

median(d::ContinuousPowerLaw) = ((α, θ) = params(d);α > 1.0 ? 2.0^(1.0/(α - 1.0)) * θ : NaN)
...
entropy(d::ContinuousPowerLaw) = ((α, θ) = params(d); log(θ / (α-1)) + 1.0 / (α-1) + 1.0)
